...
- [ ] T032 Run full suite with coverage + mypy: `uv run pytest --cov=src --cov=tests --cov-report=term-missing` and `uv run mypy src` (deliverable: 100% coverage, clean types)
